      ...
      $.getJSON(`data/${uid}/${post_id}/desc.json`, function(postData) {
        ...
        if (main_img == "none") {
          ...
          img.onload = function() {
            ...
            let mwidth = (width=="auto") ? (this.width * ($(window).height() * 0.6) / this.height + vmin * 2) : (width + $(window).width() * 0.02);
            let mheight = (height=="auto") ? (this.height * ($(window).width() * 0.44) / this.width + vmin * 2) : (height + $(window).height() * 0.02);
            $(`div.post#${uid}_${post_id} .post_content .post_attachments`).append(`<div class="atmt_img_container" style="height: ${mheight}px; width: ${mwidth}px"><div class="atmt_img_preview"><img height=${height} width=${width} src="data/${uid}/${post_id}/${main_img}"></div></div>`);
            $("div.atmt_img_preview img").click(function() {
              $(".modal").css("display", "block");
              $(".modal img").attr("src", `data/${uid}/${post_id}/${main_img}`)
            });
            $("div.atmt_img_preview img").hover(function() {
            ...
            postData.attachments.forEach((atm, i) => {
              ...
              if (IMAGE_EXT.includes(extension)) {
                ...
                if ($(".atmt_img_more").length == 0) {
                  //$(".atmt_img_preview").append(`<span class="atmt_img_more" style="transform: translate(-2vw, ${height * 0.95}px)">...more</span>`);
                }
              } else {
                if ($(`div.post#${uid}_${post_id} .post_content .post_attachments .atmt_others_container`).length == 0) {
                  $(`div.post#${uid}_${post_id} .post_content .post_attachments`).append(`<div class="atmt_others_container" style="width: ${$(window).width()*0.878-(mwidth+2*vmin)}px;"></div>`);
                }
                $(`div.post#${uid}_${post_id} .post_content .post_attachments .atmt_others_container`).append(`<div class="atmt_others" style="order: ${i};"><a href="data/${uid}/${post_id}/${atm}">${atm}</a></div>`);
              }
            });
